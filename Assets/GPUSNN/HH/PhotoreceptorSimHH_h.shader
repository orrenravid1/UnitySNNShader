Shader"CustomRenderTexture/PhotoreceptorSimHH_h"
{
// TODO:
// 1. Need to normalize and un-normalize, n,m,h, and V by their min and max values
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 alpha_h(float4 V)
            {
                return _AlphaMul * 0.07f * exp(-(V + 65.0f + _AlphaExpVoltageOffset) / (20.0f + _AlphaExpDenomOffset));
            }

            float4 beta_h(float4 V)
            {
                return _BetaMul * 1.0f / (1.0f + exp(-(V + 35.0f + _BetaExpVoltageOffset) / (10.0f + _BetaExpDenomOffset)));
            }

            float4 dhdt(float4 h_prev, float4 V_prev) 
            {
                return alpha_h(V_prev) * (1-h_prev) - beta_h(V_prev) * h_prev;
            }

            float4 HH_h(float4 h_prev, float4 V_prev)
            {
                float4 h = h_prev;
                float4 V_in = scale(V_prev, _V_Min, _V_Max);
                h += dhdt(h_prev, V_in) * _DeltaTime;
                return h;
            }

            float4 frag(v2f_customrendertexture IN) : COLOR
            {
                float4 h_prev = tex2D(_SelfTexture2D, IN.localTexcoord.xy);
                float4 V_prev = tex2D(_V_Tex, IN.localTexcoord.xy);
                return clamp(HH_h(h_prev, V_prev * _Color), 0, 1);
